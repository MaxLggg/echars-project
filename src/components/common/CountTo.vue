...
export default defineComponent({
  ...
  setup(props: ICountToProps, context: SetupContext) {
    ...
    const displayValue = ref<string>(
      formatNumber(props.startVal, props.decimals)
    ); // 具体显示的格式化后的字符串
    ...
    const count = (timestamp: number) => {
      ...
      displayValue.value = formatNumber(printVal, props.decimals);
    ...
    const reset = () => {
      startTime = 0;
      cancelAnimationFrame(requestAnimationFrameHandler);
      displayValue.value = formatNumber(props.startVal, props.decimals);
    };
